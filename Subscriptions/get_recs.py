...
for subscription in subscriptionClient.subscriptions.list():
    ...
    recommendations = advisorClient.recommendations.list()
    for r in recommendations:
        if r.category == "Security":
            continue
        if r.short_description.solution == "Follow Security Center recommendations":
            continue
        print("\t\t{} issue with {}: {}".format(r.impact, r.impacted_field, r.impacted_value))
        print("\t\t\t{}: {}".format(r.short_description.problem, r.short_description.solution))
